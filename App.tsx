...
import { Library, FlaskConical, LayoutGrid } from 'lucide-react';

const App = () => {
  const [currentMode, setCurrentMode] = useState<'library' | 'lab'>('library');
  ...
  return (
    <div className="flex h-screen w-screen bg-zinc-950 text-zinc-200 font-sans overflow-hidden selection:bg-blue-500/30">
      ...
      <div className="w-16 h-full border-r border-zinc-800 flex flex-col items-center py-6 bg-zinc-950 z-20 shrink-0">
        {/* Logo Image */}
        <div className="mb-8">
        ...
        <button 
          ...
          className={`p-3 rounded-xl mb-4 transition-all ${currentMode === 'library' ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-200 hover:bg-zinc-900'}`}
          title="模型库"
        >
          <Library size={24} />
        </button>
